            ...
            <section class="content">
                ...
                <section class="content-header">
                    <div class="container-fluid">
                        <div class="mb-2 row">
                            <div class="col-sm-6">
                                <h1>Doctors Reports</h1>
                            </div>
                            <div class="col-sm-6">
                                <ol class="breadcrumb float-sm-right">
                                    <li class="breadcrumb-item"><a href="{{ url('/dashboard') }}">Dashboard</a>
                                    </li>
                                    <li class="breadcrumb-item">Doctors Reports
                                    </li>
                                </ol>
                            </div>
                        </div>
                    ...
                    <div class="mt-3 col-md-12">
                        <div class="card ">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <h3 class="card-title">Doctor Report</h3>
                                    ...
                                    @if (auth()->user()->is_admin == '1' || Auth::user()->type == 'Admin')
                                        <div id="branchDropdown" class="dropdown ml-auto"
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                <a href="{{ url('doctor') }}" class="dropdown-item">All
                                                    Doctors</a>
                                                @foreach ($branch_drop as $drop)
                                                    <a class="dropdown-item"
                                                        href="{{ route('doctor_report', ['branch' => $drop->id]) }}">{{ $drop->name }}</a>
                                                @endforeach
                                            </div>
                                        </div>
                                    @endif
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($doctors as $doctor)
                                            <tr>
                                                ...
                                                <td><a
                                                        href="{{ url('doctorDetail', $doctor->id) }}">{{ $doctor->name }}</a>
                                                </td>
                                                <td>{{ $doctor->phno }}</td>
                                                <td>{{ $doctor->sale_commission ?? $doctor->sale_commission_percentage . '%' }}
                                                </td>
                                                <!-- <td>{{-- $doctor->created_at->format('d M Y') --}}</td> -->
                                                @php
                                                    $invoiceTotal = $invoices
                                                        ->whereIn('status', ['pos', 'invoice'])
                                                ...
                                                <td>{{ $totalInvoice }}</td>
                                                <td>
                                                    @if ($doctor->sale_commission != null)
                                                        {{ number_format($totalInvoice * $doctor->sale_commission) }}
                                                        @php
                                                            $totalamount += $totalInvoice * $doctor->sale_commission;
                                                        @endphp
                                                    @else
                                                        {{ number_format(intval(($invoiceTotal * $doctor->sale_commission_percentage) / 100)) }}
                                                        @php
                                                            $totalamount += intval(
                                                                ($invoiceTotal * $doctor->sale_commission_percentage) /
                                                                    100,
                                                            );
                                                        @endphp
                                                    @endif
                                                </td>
                                                <td>{{ number_format($invoiceTotal) }}</td>
